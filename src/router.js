...
export function createRouter({routes = []}) {
        ...
        routes: [
            {
                ...
                beforeEnter(to, from, next) {
                    ...
                    if (validateRoute(to.fullPath)) { // 跳转到有效路由
                        if(to.fullPath.endsWith('login')==false){
                            window.localStorage.setItem("appName",to.fullPath.substring(1));
                        }
    ...
    router.beforeEach((to, from, next) => {
        ...
        if (router.app.$store) {
            ...
            token = router.app.$store.state.appShell.token;
        }
        if(to.fullPath=='/home/login'){
            next();
            return;
        }
        axios(`/api/user/authenticate?token=${token}`,{method:"get",responseType:"json"}).then(function (res) {
            if(res.status===200){
                if(router.app.$store){
                    router.app.$store.commit(`appShell/${types.SET_AUTHENTICATED}`, {authenticated:res.data.authenticated,token});
